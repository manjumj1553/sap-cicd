...
locals {
  ...
  instance_mem_map = {
    ...
    "m2-hypermem-416" = 8630
    "m2-ultramem-208" = 5750
    "m2-megamem-416"  = 5750
    "m3-megamem-64"   = 976
    "m3-megamem-128"  = 1952
    "m3-ultramem-32"  = 976
  ...
  hana_shared_size = min(1024, lookup(local.instance_mem_map, var.instance_type)) - 1
  hana_usr_size    = 32
  hana_backup_size = lookup(local.instance_mem_map, var.instance_type) * 2
  pd_ssd_size      = max(lookup(local.pd_ssd_map, var.instance_type), (local.hana_log_size + local.hana_data_size + local.hana_shared_size + local.hana_usr_size)) + 2
