...
locals {
  ...
  network_parts_hana = split("/", data.google_compute_subnetwork.subnetwork.network)
  network_hana       = element(local.network_parts_hana, length(local.network_parts_hana) - 1)

  sap_hana_health_check = {
    type                = "tcp"
    ...
    response            = ""
    proxy_header        = "NONE"
    port                = var.sap_hana_health_check_port
    port_name           = var.sap_hana_health_check_port_name
    request             = ""
    request_path        = "/"
    host                = ""
  }
