...
locals {
  sap_nw_ascs_health_check = {
    ...
    response            = ""
    proxy_header        = "NONE"
    port                = var.sap_nw_ascs_health_check_port
    port_name           = var.sap_nw_ascs_health_check_port_name #"health-check-port"
    request             = ""
    request_path        = "/"
    host                = ""
  }
  sap_nw_ers_health_check = {
    type                = "tcp"
    check_interval_sec  = 10
    healthy_threshold   = 4
    timeout_sec         = 10
    ...
    response            = ""
    proxy_header        = "NONE"
    port                = var.sap_nw_ers_health_check_port
    port_name           = var.sap_nw_ers_health_check_port_name #"health-check-port"
    request             = ""
    request_path        = "/"
    host                = ""
  }
