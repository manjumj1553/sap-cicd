...
module "gcp_netweaver" {
  source                = "../../../terraform/modules/nw-standard-aas-lite"
  ...
  zone                  = var.sap_nw_aas_zone
  instance_name         = var.sap_nw_aas_instance_name
  instance_ip           = var.sap_nw_aas_instance_ip
  instance_type         = var.sap_nw_aas_instance_type
  use_public_ip         = var.sap_nw_aas_use_public_ip
  ...
  alias_ip_inp          = var.sap_nw_aas_alias_ip
  target_size           = length(var.sap_nw_aas_instance_ip)
  alias_ip_range = {
    ip_cidr_range = "/32"
  }
}